...
| `x-nx-launchpad` | Fullscreen App-Launcher. `modules` (key/title/icon/url/**group**/badge?) · `anchors` (neutrale Reihe oben) · `favorites` (meistgenutzt, farbige Zeile) · `hotkey` (default 'l' → ⌘/Strg+⇧+L, toggelt) oder Event `open-launchpad`. Aufbau: Anker (neutral) · Favoriten · Alle Module. Kachelfarbe = f(Gruppe) stabil (--nx-tone-*), Icon in Gruppenfarbe, Monogramm-Fallback, Suche. Kachel-Partial: `launchpad/_tile`. In core via `@livewire('core.launchpad')`; Anker aus `config('platform.launchpad.anchors')`, Favoriten aus `ModuleUsageCount` |
