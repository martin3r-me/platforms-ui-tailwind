<div class="flex h-full">
    ...
    <div class="flex-1 flex flex-col">
        <!-- Header oben (fix) -->
        <div class="border-t border-b border-[color:var(--ui-border)] p-2 shrink-0">
            <div class="flex gap-1">
                <div class="flex">
                    @foreach($breadcrumbItems as $item)
                        @if($item)
                            @if(isset($item['current']) && $item['current'])
                                <span class="px-3 border-r border-[color:var(--ui-border)] font-semibold">
                                    {{ $item['label'] }}
                                </span>
                            @else
                                <a href="{{ $item['href'] }}" class="px-3 border-r border-[color:var(--ui-border)] underline" wire:navigate>
                                    {{ $item['label'] }}
                                </a>
                            @endif
                        @endif
        ...
        <div class="flex-1 overflow-y-auto p-4">
            {{-- Aufgaben-Details --}}
            <div class="mb-6">
                <h3 class="text-lg font-semibold mb-4 text-[color:var(--ui-secondary)]">Aufgaben-Details</h3>
            ...
            {{-- Status & Zuweisung --}}
            <div class="mb-6">
                <h3 class="text-lg font-semibold mb-4 text-[color:var(--ui-secondary)]">Status & Zuweisung</h3>
                <div class="grid grid-cols-2 gap-4">
                    <x-ui-input-checkbox
                        model="task.is_done"
        ...
        <div class="shrink-0 border-t border-[color:var(--ui-border)]">
            <div 
                @click="open = !open" 
                class="cursor-pointer border-t border-b border-[color:var(--ui-border)] p-2 text-center flex items-center justify-center gap-1 mx-2 shadow-lg"
                x-data="{ open: false }"
            >
                AKTIVITÄTEN 
                <span class="text-xs">
    ...
    <!-- Rechte Spalte -->
    <div class="min-w-80 w-80 flex flex-col border-l border-[color:var(--ui-border)]">
        <div class="flex gap-2 border-t border-b border-[color:var(--ui-border)] p-2 shrink-0">
        ...
        <div class="flex-1 overflow-y-auto p-4">
            ...
            <div class="mb-4 p-3 bg-[color:var(--ui-muted-5)] rounded-lg">
                <h4 class="font-semibold mb-2 text-[color:var(--ui-secondary)]">Aufgaben-Übersicht</h4>
